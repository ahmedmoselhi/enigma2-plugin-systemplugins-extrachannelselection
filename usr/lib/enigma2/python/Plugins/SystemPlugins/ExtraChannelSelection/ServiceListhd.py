...
# Compiled at: 2019-08-31 11:39:05
...
class ServiceList(HTMLComponent, GUIComponent):
    ...
    def __init__(self, serviceList):
        ...
        self.serviceEventProgressbarBorderColor = 16777215
        ...
        self.l.setFont(3, gFont('Regular', 25))
        self.l.setFont(4, gFont('Regular', 25))
        self.l.setFont(5, gFont('Regular', 25))
        self.l.setFont(6, gFont('Regular', 25))
        self.l.setFont(10, gFont('Regular', 25))
        self.l.setFont(7, gFont('Regular', 30))
        self.l.setFont(8, gFont('Regular', 25))
        self.l.setFont(9, gFont('Regular', 25))
        self.l.setFont(11, gFont('Regular', 25))
        self.l.setFont(12, gFont('Regular', 25))
        self.l.setFont(13, gFont('Regular', 25))
        self.list = []
        self.size = 0
    ...
    def buildServiceList(self, service, **args):
        ...
        self.colordict = {'1': 1813988, '2': 25799, 
           '3': 8421504, 
           '4': 20928, 
           '5': 27131, 
           '6': 65535, 
           ...
           '46': 4227693, 
           '47': 16034048}
        self.picdict = {'1': (self.picBar), '2': (self.picBar11), 
           '3': (self.picBar1), 
           '4': (self.picBar2), 
           '5': (self.picBar3), 
           '6': (self.picBar4), 
           ...
           '11': (self.picBar9), 
           '12': (self.picBar10)}
        self.pixdict = {'1': (self.Bar), '2': (self.Bar11), 
           '3': (self.Bar1), 
           '4': (self.Bar2), 
           '5': (self.Bar3), 
           '6': (self.Bar4), 
        ...
        if not nummode:
            ...
            if not service.flags & eServiceReference.isMarker:
                ...
                num = '%d' % service.getChannelNum()
                numCol = 16711680 if isRecorded else numColor
                numColSel = 16711680 if isRecorded else numColorSel
                if DOUBLE:
                    if recordmode == '3':
                        addingResText(xPos + 0, -2, 87, height, 7, rthcrtvc, num, numCol, numColSel, backgroundColor, backgroundColorSel)
                        xPos += 93
                        xPos1 = xPos
                        xPos2 = xPos
                    else:
                        addingResText(xPos + 0, -2, 87, height, 7, rthcrtvc, num, numColor, numColorSel, backgroundColor, backgroundColorSel)
                        xPos += 93
                        xPos1 = xPos
                        xPos2 = xPos
                elif recordmode == '3':
        ...
        if not notChannelMode:
            ...
            if DOUBLE:
                ...
                if isPlayable:
                    ...
                    if not DOUBL:
                        ...
                        if barmode == '1':
                            addingResPr(950, 15, 75, 12, bar, 1, barColor, barColorSel, backgroundColor, backgroundColorSel)
                            xPos += 85
                        else:
                            addingResPrPix(xPos + 3, 13, 75, 12, bar, pixbar, 1, borderColor, borderColorSel)
                            xPos += 85
                        if percshow:
                            addingResText(1015, 2, 95, height / 2, 1, rthcrtvc, perc, percColor, percColorSel, backgroundColor, backgroundColorSel)
                            xPos += 105
                        if crypticonmode == '1':
                            if isCrypted:
                        ...
                        if timericonmode == '1':
                            if isWaitingTimer:
                                ...
                                if pixtimer_height < height / 2:
                                    ...
                                    jjk = 0
                                    hhj = height / 2
                                addingResPix(207, 5, 200, 30, self.timerclock)
                        ...
                        if recordmode == '1':
                            if isRecorded:
                                ...
                                if pixrec_height < height / 2:
                                    ...
                                    jjk = 0
                                    hhj = height / 2
                                addingResPix(210, jjk, pixrec_width, hhj, self.picRecord)
                                xPos += pixrec_width + 15
                        nameCol = 16711680 if isRecorded else nameColor
                        nameColSel = 16711680 if isRecorded else nameColorSel
                        addingResText(xPos - 190, 2, length, yDouble - 4, 2, rthlrtvc, serviceName, nameCol, nameColSel, backgroundColor, backgroundColorSel)
                        xPos += length + 5
                        if pixico is not None:
                            if iconmode == '2':
                                pixico_size = pixico.size()
                                ...
                                addingResPix(1131, lgh, 45, igj, pixico)
                                xPos += 65
                        if crypticonmode == '2':
                            if isCrypted:
                                cryptpixmap_size = icocrypt.size()
                                ...
                                addingResPix(1090, iii, pixx_width, eee, icocrypt)
                                xPos += pixx_width + 15
                        if timericonmode == '2':
                            if isWaitingTimer:
                                timerpixmap_size = self.timerclock.size()
                                ...
                                if pixtimer_height < height / 2:
                                    ...
                                    jjk = 0
                                    hhj = height / 2
                                addingResPix(82, 5, 200, 30, self.timerclock)
                                xPos += pixtimer_width + 15
                        if recordmode == '2':
                        ...
                        if remmode != '3' and event:
                            addingResText(780, 2, 180, 36, 5, rthcrtvc, remain, remColor, remColorSel, None, None)
                        xPos1 += 195
                        xPos2 += 195
                        if endmode and event:
                            ...
                            if piconmode:
                                ...
                                ff = 500
                            addingResText(619, 3, ff, yDouble - 5, 4, rthlrtvc, '%02d.%02d - %02d.%02d' % (begin[3],
                             begin[4],
                             end[3],
                        ...
                        if timericonmode == '1':
                            if isWaitingTimer:
                                ...
                                if pixtimer_height < height / 2:
                                    ...
                                    jjk = 0
                                    hhj = height / 2
                                    addingResPix(2, jjk, pixtimer_width, hhj, self.timerclock)
                                xPos += pixtimer_width + 15
                        if recordmode == '1':
                            if isRecorded:
                                recpixmap_size = self.picRecord.size()
                        ...
                        nameCol = 16711680 if isRecorded else nameColor
                        nameColSel = 16711680 if isRecorded else nameColorSel
                        addingResText(160, 2, jj, 36, 2, rthlrtvc, serviceName, nameCol, nameColSel, backgroundColor, backgroundColorSel)
                        xPos += jj + 15
                        if pixico is not None:
                            if iconmode == '2':
                                pixico_size = pixico.size()
            ...
            if piconmode:
                ...
                if barmode == '1' and usagemode != 'no':
                    ...
                    if not barpercmode:
                        ww = (height - 12) / 2 if 1 else height - 16
                    ...
                    if timericonmode == '1':
                        if isWaitingTimer:
                            ...
                            addingResPix(2, www, pixtimer_width, pixtimer_height, self.timerclock)
                            xPos += pixtimer_width + 15
                    if recordmode == '1':
                        if isRecorded:
                            recpixmap_size = self.picRecord.size()
        ...
        if event and isPlayable:
            ...
            if DOUBLE:
                if not DOUBL:
                    if piconmode:
                        ...
                        hh = width - xPos1 - 5
                    addingResText(xPos1 - 195, yDouble + 1, 1000, yDouble - 4, 3, rthlrtvc, eventname, eventColor, eventColorSel, backgroundColor, backgroundColorSel)
                else:
                    if piconmode:
                        if picomode == '1':
                            bn = width - xPos1 - 15
            ...
            if notChannelMode:
                addingResText(xPos + 10, -5, width - xPos - 20, 86, 10, rthlrtvc, serviceName, satColor, satColorSel, backgroundColor, backgroundColorSel)
            elif text:
                if DOUBL:
                    nn = 27
                    if piconmode:
                    ...
                    qw = 40
                    we = 12
                addingResText(205, nn, mm, qw, we, rthlrtvc, notEpg, txtColor, 16444375, backgroundColor, backgroundColorSel)
        else:
            self.renderLabel.setFont(self.ServiceNameFont)
            self.renderLabel.setText(serviceName)
            length = self.renderLabel.calculateSize().width() + 10
...
# okay decompiling ./ServiceListhd.pyo
