...
# Embedded file name: /usr/lib/enigma2/python/Plugins/SystemPlugins/ExtraChannelSelection/ChannelSelection.py
# Compiled at: 2019-08-31 08:36:44
from boxbranding import getMachineBuild, getMachineBrand, getMachineName
import os
from Tools.Profile import profile
from Screens.Screen import Screen
import Screens.InfoBar, Components.ParentalControl
from Components.Button import Button
from ServiceList import ServiceList, refreshServiceList
...
class ChannelContextMenu(Screen):
    ...
    def __init__(self, session, csel):
        ...
        self['actions'] = ActionMap(['OkCancelActions',
         'ColorActions',
         'NumberActions',
         'MenuActions'], {'ok': (self.okbuttonClick), 'cancel': (self.cancelClick), 
        ...
        if csel.bouquet_mark_edit == OFF and not csel.entry_marked:
            if not inBouquetRootList:
                ...
                elif 'FROM SATELLITES' in current_root.getPath() and current and _('Services') in eServiceCenter.getInstance().info(current).getName(current):
    ...
    def removeEntry(self):
        ...
        if self.removeFunction and ref and ref.valid():
            if self.csel.confirmRemove:
                list = [
                 (
                  _('yes'), True), (_('no'), False), (_('yes') + ', ' + _('and never ask again this session again'), 'never')]
...
class ChannelSelectionEPG(InfoBarButtonSetup):
    ...
    def __init__(self):
        ...
        self['recordingactions'] = HelpableActionMap(self, 'InfobarInstantRecord', {'ShortRecord': (self.RecordTimerQuestion, _('Add a record timer')), 'LongRecord': (
                        self.doZapTimer, _('Add a zap timer for next event'))}, -1)
        self['dialogactions'] = ActionMap(['SetupActions'], {'cancel': (self.closeChoiceBoxDialog)})
            ...
            if eventidnext is not None:
                menu = [
                 (
                  ...
                  'CALLFUNC',
                  self.ChoiceBoxCB,
                  self.doRecordCurrentTimer),
                 (menu2,
                  'CALLFUNC',
                  self.ChoiceBoxCB,
                  self.doRecordNextTimer)]
            else:
                menu = [
                 (
                  menu1,
...
class ChannelSelectionBase(Screen):
    ...
    def __init__(self, session):
        ...
        self['ChannelSelectBaseActions'] = NumberActionMap(['ChannelSelectBaseActions', 'NumberActions', 'InputAsciiActions'], {'ServiceDown': (self.moveDown), 'ServiceUp': (self.moveUp), 
        ...
        if not self.pathChangeDisabled:
            ...
            if not self.preEnterPath(refstr):
                ...
                if justSet:
                    ...
                    serviceHandler = eServiceCenter.getInstance()
                    if config.plugins.ExtraChannelSelection.showhd.value:
                        if self.mode == 0:
                            typeslist = ('1:7:1:0:0:0:0:0:0:0:(type == 1) || (type == 4) || (type == 22) || (type == 24)',
                                         '1:7:11:0:0:0:0:0:0:0:(type == 17) || (type == 25) || (type == 27) || (type == 31) || (type == 134) || (type == 195)')
                        else:
                        ...
                        for srvtypes in typeslist:
                            ...
                            if servicelist is not None:
                                while True:
                                    ...
                                    if orbpos < 0:
                                        orbpos += 3600
                                    if config.plugins.ExtraChannelSelection.showprov.value:
                                        if config.plugins.ExtraChannelSelection.shownew.value:
                                        ...
                                        elif service.getPath().find('FROM PROVIDER') != -1:
                                            service_type = _('Providers')
                                        elif service.getPath().find('flags == %d' % FLAG_SERVICE_NEW_FOUND) != -1:
                                            service_type = 0
                                        else:
                                            service_type = _('Services')
                                    elif config.plugins.ExtraChannelSelection.shownew.value:
                                        if service.getPath().find('FROM PROVIDER') != -1:
                                            service_type = 0
                                        elif service.getPath().find('flags == %d' % FLAG_SERVICE_NEW_FOUND) != -1:
                                            service_type = _('New')
                                    ...
                                    if service_type:
                                        ...
                                        if ('(type == 1)' or '(type == 4)' or '(type == 22)' or '(type == 24)') in srvtypes:
                                            service_type = 'SD-%s' % service_type
                                        elif ('(type == 17)' or '(type == 25)' or '(type == 27)' or '(type == 31)' or '(type == 134)' or '(type == 195)') in srvtypes:
                                            service_type = 'HD-%s' % service_type
                                        if config.plugins.ExtraChannelSelection.channsum.value:
                    ...
                    if cur_ref:
                        ...
                        ref = eServiceReference(refstr)
                        if config.plugins.ExtraChannelSelection.showcurr.value:
                            if config.plugins.ExtraChannelSelection.channsum.value:
                    ...
                    if hdref:
                        if config.plugins.ExtraChannelSelection.showsky.value:
                            if config.plugins.ExtraChannelSelection.channsum.value:
                                hdref.setName('%s - %s' % ('Sky', _('Subservices') + ' (%d)' % self.getServicesCount(hdref)))
                            else:
                                hdref.setName('%s - %s' % ('Sky', _('Subservices')))
    ...
    def nextBouquet(self):
        if 'reverseB' in config.usage.servicelist_cursor_behavior.value:
            if config.usage.channelbutton_mode.value == '0' or config.usage.channelbutton_mode.value == '3':
                ...
                self.servicelist.moveDown()
        elif config.usage.channelbutton_mode.value == '0' or config.usage.channelbutton_mode.value == '3':
            self.changeBouquet(+1)
        else:
            self.servicelist.moveUp()
        return
...
class ChannelSelection(ChannelSelectionBase, ChannelSelectionEdit, ChannelSelectionEPG, SelectionEventInfo):
    ...
    def __init__(self, session):
        ...
        if config.usage.servicelist_mode.value == 'simple':
            self.skinName = [
             'SlimChannelSelection', 'SimpleChannelSelection', 'ChannelSelection']
        else:
            self.skinName = 'ChannelSelection'
        self['actions'] = ActionMap(['OkCancelActions', 'TvRadioActions'], {'cancel': (self.cancel), 'ok': (self.channelSelected), 
           'keyRadio': (self.toogleTvRadio), 
           'keyTV': (self.toogleTvRadio)})
        self.radioTV = 0
        self.__event_tracker = ServiceEventTracker(screen=self, eventmap={(iPlayableService.evStart): (self.__evServiceStart), (iPlayableService.evEnd): (self.__evServiceEnd)})
...
class ChannelSelectionRadio(ChannelSelectionBase, ChannelSelectionEdit, ChannelSelectionEPG, InfoBarBase, SelectionEventInfo):
    ...
    def __init__(self, session, infobar):
        ...
        self['actions'] = ActionMap(['OkCancelActions', 'TvRadioActions'], {'keyTV': (self.cancel), 'keyRadio': (self.cancel), 
           'cancel': (self.cancel), 
...
class SimpleChannelSelection(ChannelSelectionBase):
    ...
    def __init__(self, session, title, currentBouquet=False):
        ChannelSelectionBase.__init__(self, session)
        self['actions'] = ActionMap(['OkCancelActions', 'TvRadioActions'], {'cancel': (self.close), 'ok': (self.channelSelected), 
...
class HistoryZapSelector(Screen):
    ...
    def __init__(self, session, items=None, sel_item=0, mark_item=0, invert_items=False, redirect_buttons=False, wrap_around=True):
        ...
        self['actions'] = ActionMap(['OkCancelActions', 'InfobarCueSheetActions'], {'ok': (self.okbuttonClick), 'cancel': (self.cancelClick), 
...
# okay decompiling ./ChannelSelection.pyo
